...
bool create_gl_context(Internal_State *state);

internal bool 
window_initialize(Window_State *window, const char *title,
...
bool 
create_gl_context(Internal_State *state) 
{
...
window_poll_message() 
{
    i32 result = 0;
    MSG msg;
    while ((result = PeekMessageA(&msg, NULL, 0, 0, PM_REMOVE) != 0) && result != -1) 
    {
