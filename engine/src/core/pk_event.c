...
global event_system_state* g_state = (void*)0;
...
event_initialize() {
    ...
    g_state = memory_allocate(sizeof(event_system_state), MEMORY_TAG_APPLICATION);
...
b8 event_register(u16 code, void* listener, on_event on_event) {
    if (!g_state) {
        return false;
    }
...
b8 event_unregister(u16 code, void* listener, on_event on_event) {
    if (!g_state) {
        return false;
    }
